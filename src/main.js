...
const TeraGuide = (dispatch) => {
    ...
    function handle_event(ent, id, called_from_identifier, prefix_identifier, d, speed=1.0, stage=false) {
        ...
        if(entry) start_events(entry, ent, speed);
        if(active_guide.onEvent) start_events(active_guide.onEvent, ent, speed, prefix_identifier, id);
    }

    // This is where all the magic happens
    function start_events(events=[], ent, speed=1.0, ...args) {
        ...
        for(let event of events) {
            ...
            else if(class_position_check(event['class_position'])) func(event, ent, speed=1.0, ...args);
        }
    }
    ...
    // Func handler
    function func_handler(event, ent, speed=1.0, ...args) {
        ...
        timers[event['id'] || random_timer_id--] = setTimeout(event['func'], (event['delay'] || 0) / speed, function_event_handlers, event, ent, fake_dispatch, ...args);
    }

    function enable() {
        hooks.push(dispatch.hook('S_ACTION_STAGE', 9, {order: 15}, s_action_stage));
        hooks.push(dispatch.hook('S_ABNORMALITY_BEGIN', 3, {order: 15}, abnormality_triggered));
        hooks.push(dispatch.hook('S_ABNORMALITY_REFRESH', 1, {order: 15}, abnormality_triggered));
        hooks.push(dispatch.hook('S_BOSS_GAGE_INFO', 3, boss_hp_bar_trigger));
        hooks.push(dispatch.hook('S_DUNGEON_EVENT_MESSAGE', 2, dungeon_event_message));
        hooks.push(dispatch.hook('S_QUEST_BALLOON', 1, quest_balloon));
    }

    function disable() {
        for(const hook of hooks) dispatch.unhook(hook);
        hooks = [];
    }
}
